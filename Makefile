...
cmd/wal-g/wal-g: $(CMD_FILES) $(PKG_FILES)
	( echo `date -u +%Y.%m.%d_%H:%M:%S` && echo `git rev-parse --short HEAD` && echo `git tag -l --points-at HEAD` && cd cmd/wal-g && go build $(GOTAGS) -ldflags "-s -w -X main.BuildDate=`date -u +%Y.%m.%d_%H:%M:%S` -X main.GitRevision=`git rev-parse --short HEAD` -X main.WalgVersion=`git tag -l --points-at HEAD`")
